...
def init_unlocked(level):
    ...
    ⚠ `sms` / `eggs` 两列现在的语义**都是「已解锁」**（2026-09-24 起 QQ 端一条都不发了）：
      `sms` 只喂网页端 `/messages`；`eggs` 喂网页端 `/me` 的「他说过的那句话」，
      同时兼作「处理过哪些」的去重表（`pending_unlock` 里的 `sent_eggs`）。
      ⭐ 历史遗留：2026-09-24 之前 `eggs` 有一半语义是「**真发过**」⇒ 老数据里那部分
      确实是他亲口说过的；新数据就只剩「到级」。她 09-21 拍过「到级就算说过了」，
      所以网页端那张卡**不改**（这个缺口记在 `compute()` 里，别当 bug 去修）。
    """
    lv = int(level or 1)
    return {"level": lv,
...
def pending_unlock(user_id, memory_dir):
    ...
    ⭐⭐ **QQ 端一条都不发**（两批素材都取消了主动发送）：
       · 牵绊短信 —— 2026-09-21 她定：「既然写进了网页端，就不放在 QQ 对话端里了」；
       · 彩蛋 —— 2026-09-24 她定：跨级时突然冒出一条官方原文会**打断正在进行的对话**。
       ⇒ `send` 照样算出来（=「这级该是哪条彩蛋」），但**发不发由 bot 侧的
         `AFFINITY_UNLOCK_SEND` 决定**（现在是 False）。本函数**只做决策、不发送**。
       ⇒ 连带变化：以前「短信优先、把撞车的彩蛋挤到下次」那套**没了** ——
         彩蛋不再被挤，按 `EGG_AT` 逐个走到（7/13/33/53… 那 18 个撞车等级不再顺延）。
    ⚠ 没升级就返回 None（**绝不补发历史**）。
    ⚠ 没处理过的彩蛋**不丢**：`eggs` 只在调用方记账时才记 ⇒ 下次升级接着补。
      （跨级但不发的那些也一样记 —— 网页端那张卡按「到级就算说过」的口径读它。）
    """
    ...
    # ② 彩蛋：算出「该轮到哪一条」（发不发由 bot 侧 `AFFINITY_UNLOCK_SEND` 决定）
...
def compute(user_id, memory_dir):
    ...
    #    ⚠⚠ 一个诚实的边界（**别对外说满**）：`unlocked["eggs"]` 里混着**两批来源** ——
    #       · 首次接入：`init_unlocked()` 把**到级的全标上**（老用户不补发历史）
    #       · 跨级解锁：`pending_unlock()` 每跨一级标一条
    #       · （2026-09-24 之前还有一部分是「**真在 QQ 发出去过**」的；那天起取消了主动发送）
    #       ⇒ 严格讲这张卡是「**到级**」，不是 100%「他亲口说过」。
    #       想一分不差地只算「真说过」，得另开一个 `said` 列表（首连留空、发送时才追加）。
    #       ⭐⭐ **2026-09-21 她拍板：走 B —— 不加 `said`**，就照现在这样，
    #          「**到级就算说过了**」（理由：老用户逻辑上早该说过；测试期新用户影响也小）。
    #       ⭐ 2026-09-24 取消 QQ 主动发送时，她明确选了「**这张卡照旧随升级更新**」
    #          ⇒ 卡片继续长，只是内容变成纯「到级」。**以后别把这里当 bug 去修**。
    #          真要改口径，先回去看 §19.7。
