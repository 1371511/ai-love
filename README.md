...
- 🧭 **底部那一条**（每页一条，**不是全站共用同一条**）：`/me` = 设置 · 退出；`/messages` = 回去；`/messages/{等级}` = ↺ 从头再聊一遍 · 回列表；`/settings` = 回去。
  ⚠⭐ 2026-09-21 她定：**钉在屏幕底部**（`.footnav{position:fixed;…;bottom:0}`）—— 页面一长它跟着滚走就找不着了（原话「让它一直保持在界面里」）。
  ⚠ **内容一个字都没加** —— 她之前定的「底部只留设置 / 退出」没动；延续之前那条：**别把「牵绊短信」塞进底部。**
  ⚠⚠ `body{padding-bottom:56px}` 是给它**留位**的：`position:fixed` 脱离文档流，不留位最后一屏会被压住 ⇒ **改 `.footnav` 的高度必须同步改它**。
