...
| 触发 | 发什么 |
|---|---|
| 该等级是 **45 个短信节点**之一（7 / 13 / 16 / … / 166） | 那条短信的**开头句**（`card/affinity/牵绊短信/`）。整条是带 A/B/C 分支的多轮对话，QQ 里发不全，**只发他开口的第一句** |
| 其他等级 | 86 条彩蛋里**绑在该等级**上的那条（等级映射在 `card/affinity.md` 的 `EGG_AT=`，短句递进，约每 2.9 级一条） |

- ⭐ **一级最多一条、短信优先于彩蛋**；被挤掉的那条彩蛋**不丢**，下次升级补上。
- ⭐ **每用户每条只发一次**（记在 `memory/{uid}_daily.json` 的 `unlocked`）；
  **老用户首次接入不补发历史**（只记当前等级）—— 否则一上来就被灌二十多条。
- ⚠ 只在**私聊**里发；发出去的话同样走 `record_proactive` 写进对话记忆。
- ⚠ 素材里的 `[表情：涂鸦叽：生气]` 归一成 `[表情:生气]`；`[链接：…]` 跳过取下一句；
  `用户` 这种占位符换成**她的称呼**。
- ⭐ 解锁过的会出现在网页端 **「他说过的那句话」**（有内容才渲染，纯表情那几条不上卡）。
