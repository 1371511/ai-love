...
| `AUTO_GREET_IDLE_HOURS` | 6 | 冷场多久他才忍不住 |
| `AUTO_GREET_MAX_PER_DAY` | 1 | 每天最多主动发几条（保险丝） |
| `AUTO_GREET_GAP_DAYS_MIN` / `_MAX` | 2 / 3 | 🎲 排期：两次之间隔几个自然日（在此区间随机抽） |
...
**频率**：每次发完就随机约好下一次——隔 **2~3 个自然日**、时刻在 8:00~22:59 随机一分钟，
所以实际大约**每周 2~3 次**，也不会总是同一个钟点。排期存在 `memory/{uid}_greet.json` 的 `next_at` 里。
