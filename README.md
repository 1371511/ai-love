...
- `/me`：好感度 + 档位进度、`聊过 N 轮` / `已用额度 X 万 token`、**你们之间**（画像关键词）、
  **他说过的那句话**、**牵绊短信**入口、设置
  ...
  而 86 条彩蛋按她的口径**根本不进网页端**（只走 QQ）⇒ 原来的名字「彩蛋」名实不符。
...
| 该等级是 **45 个短信节点**之一（7 / 13 / 16 / … / 166） | ⚠⭐ **一条都不发**。短信**只标「已解锁」**，正文留在**网页端**（她定：「既然写进了网页端，就不放在 QQ 对话端里了」） |
